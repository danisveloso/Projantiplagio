...
class SubCategory(models.Model):
    ...
    category = models.ForeignKey(
        ...
        verbose_name=_("Categoria"), on_delete=models.PROTECT)
...
class Quiz(models.Model):
    ...
    description = models.TextField(
        ...
        blank=True, help_text=_("Descrição do questionário"))

    url = models.SlugField(
        max_length=60, blank=False,
        help_text=_("Uma URL amigável"),
        verbose_name=_("Uma URL amigável"))
    ...
    random_order = models.BooleanField(
        ...
        help_text=_("Exibe as peguntas em"
                    "uma ordem aleatória ou como eles "
                    "estão setados?"))

    max_questions = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Questões Max"),
        help_text=_("Número de perguntas a serem respondidas em cada tentativa."))

    answers_at_end = models.BooleanField(
        blank=False, default=False,
        help_text=_("A resposta correta NÃO é mostrada após a pergunta ."
                    " Respostas exibidas no final."),
        verbose_name=_("Respostas no final"))

    exam_paper = models.BooleanField(
        blank=False, default=False,
        help_text=_("Se sim o resultado de cada"
                    " tentativa do usuário será"
                    " armazenado. Necessário marcação"),
        verbose_name=_("Resultado teste"))

    single_attempt = models.BooleanField(
        blank=False, default=False,
        help_text=_("Se sim, apenas uma tentativa por"
                    " usuário será permitida"
                    " Não usuários não poderam fazer este teste."),
        verbose_name=_("Tentativa única"))

    pass_mark = models.SmallIntegerField(
        blank=True, default=0,
        verbose_name=_("Marca de aprovação"),
        help_text=_("Porcentagem necessária para passar no teste."),
        validators=[MaxValueValidator(100)])

    success_text = models.TextField(
        blank=True, help_text=_("Exibido se o usuário for aprovado."),
        verbose_name=_("Texto de sucesso"))

    fail_text = models.TextField(
        verbose_name=_("Texto de falha"),
        blank=True, help_text=_("Exibido se o usuário falhar"))

    draft = models.BooleanField(
        blank=True, default=False,
        verbose_name=_("Draft"),
        help_text=_("Se sim o questionário não é exibido"
                    " na lista de perguntas e só pode ser"
                    " visualizada por usuários que podem editar"
                    " os testes."))
...
class Progress(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Registros de progresso do usuário")
    ...
    def update_score(self, question, score_to_add=0, possible_to_add=0):
        ...
        if any([item is False for item in [category_test,
            ...
            return _("error"), _("categoria não existe ou pontuação inválida")
...
class SittingManager(models.Manager):
    ...
    def new_sitting(self, user, quiz):
        ...
        if len(question_set) == 0:
            raise ImproperlyConfigured('O conjunto de perguntas do questionário está vazio. '
                                       'Por favor configure as perguntas corretamente')
...
class Sitting(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_("Usuário"), on_delete=models.PROTECT)

    quiz = models.ForeignKey(Quiz, verbose_name=_("Quiz"), on_delete=models.PROTECT)

    question_order = models.CharField(
        max_length=1024,
        verbose_name=_("Ordem das questões"),
        validators=[validate_comma_separated_integer_list])

    question_list = models.CharField(
        max_length=1024,
        verbose_name=_("Lista das questões"),
        validators=[validate_comma_separated_integer_list])

    incorrect_questions = models.CharField(
        max_length=1024,
        blank=True,
        verbose_name=_("Questões incorretas"),
        validators=[validate_comma_separated_integer_list])

    current_score = models.IntegerField(verbose_name=_("Pontuação atual"))

    complete = models.BooleanField(default=False, blank=False,
                                   verbose_name=_("Completa"))
    ...
    end = models.DateTimeField(null=True, blank=True, verbose_name=_("Fim"))

    objects = SittingManager()

    class Meta:
        permissions = (("view_sittings", _("Visualizar testes concluídos.")),)
...
class Question(models.Model):
    ...
    explanation = models.TextField(max_length=2000,
                                   blank=True,
                                   help_text=_("Explicação a ser mostrada"
                                               "após a pergunta "
                                               "ter sido respondida"),
